# -*- coding: utf-8 -*-
"""
Created on Thu Jun 12 14:25:18 2025
...
if data_type==1:
    ...
    #model = Pipeline(steps=[('scaler', StandardScaler()), ('gnb', GaussianNB(var_smoothing=1e-10))])
...
if smotenc_use == 0:
    ...
    if scaler_use == 0:
        ...
        X_test_scaled =X_test_con_scaled.join(X_test_cat)   
...
if tuning ==1:
    ...
    # 数据预处理与模型组合 
    #pipeline = Pipeline([ ('scaler', StandardScaler()), # 标准化（非必须，但可优化数值稳定性）    
           #     ('gnb', GaussianNB()) ]) 
    
    # 定义参数网格 
    #param_grid = {'gnb__var_smoothing': [1e-10, 1e-9, 1e-8, 1e-7]} 
    param_grid = {'var_smoothing': [1e-12, 1e-9, 1e-6, 1e-3]}
    
    # 执行网格搜索 
    grid_search = GridSearchCV(GaussianNB(), param_grid, cv=5, scoring='f1')  # scoring='f1' 和scoring='accuracy' 一样
